...
fn build_conversion_rates(base_rates : &[(Currency, Currency, f32)]) -> HashMap<Currency, HashMap<Currency, f32>> {
    ...
    base_rates
        ...
        .for_each(|(first_currency, second_currency, rate)| {
            ...
            let mappings_for_second_currency = map.entry(second_currency.clone()).or_insert(HashMap::new());
        ...
        F : FnMut(&str) -> Option<T>
{
    text
        .split("\n")
        .map(str_to_T)
        .partition(|e| e.is_some())
}

fn parse_transactions(text : &String) -> Vec<Transaction> {
    let (valid, error) = process_lines(
        text, 
        |line| {
...
fn process_transactions(accounts : &mut HashMap<String, Account>, conversion_rates : &HashMap<Currency, HashMap<Currency, f32>>, transactions : &Vec<Transaction>) {
    ...
    transactions
        ...
        .for_each(|transaction| {
            ...
            match accounts.get(account_number) {
                Some(account) => {
                    match apply_transaction(account, transaction, conversion_rates) {
                        Some(updated_account) => {
                            accounts.insert(account_number.to_string(), updated_account);
                            ()
                        },
                        None => println!("Failed to apply transaction {:?} to {}", transaction, account_number)
                    }
                }
                None => ()
            }
...
fn apply_transaction(account : &Account, transaction : &Transaction, currency_conversion : &HashMap<Currency, HashMap<Currency, f32>>) -> Option<Account> {
    let (transaction_currency, transaction_amount) = 
        match transaction {
...
fn apply_money(Money {amount, currency} : &Money, amount_to_apply : f32) -> Money {
    Money { amount: amount + amount_to_apply, currency: *currency }
}